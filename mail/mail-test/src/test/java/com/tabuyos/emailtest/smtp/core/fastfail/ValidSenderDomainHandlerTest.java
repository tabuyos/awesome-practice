...
public class ValidSenderDomainHandlerTest {
  ...
  void testNullSenderNotReject() {
    ...
    HookReturnCode response =
        handler.doMail(setupMockedSession(null), MaybeSender.nullSender()).getResult();

    assertThat(HookReturnCode.declined())
        .describedAs("Not blocked cause its a nullsender")
        .isEqualTo(response);
  }
  ...
  void testInvalidSenderDomainReject() throws Exception {
    ...
    assertThat(HookReturnCode.deny())
        .describedAs("Blocked cause we use reject action")
        .isEqualTo(response);
  }
}
