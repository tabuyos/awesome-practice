...
public class ResolvableEhloHeloHandlerTest {
  ...
  private SMTPSession setupMockSession(
      String argument,
      final boolean relaying,
  ...
  void testRejectInvalidHelo() throws Exception {
    ...
    assertThat(session.getAttachment(BAD_EHLO_HELO, Transaction))
        .withFailMessage("Invalid HELO")
        .isPresent();

    HookReturnCode result =
        handler.doRcpt(session, MaybeSender.nullSender(), mailAddress).getResult();
  ...
  void testRejectInvalidHeloAuthUser() throws Exception {
    MailAddress mailAddress = new MailAddress("test@localhost");
    SMTPSession session =
        setupMockSession(INVALID_HOST, false, true, Username.of("valid@user"), mailAddress);
    ResolvableEhloHeloHandler handler = createHandler();

    handler.doHelo(session, INVALID_HOST);
    assertThat(session.getAttachment(BAD_EHLO_HELO, Transaction))
        .withFailMessage("Value stored")
        .isPresent();
