 ...
 * under the License.                                           *
 ****************************************************************/
package com.tabuyos.emailtest.smtp.netty;
...
public class NettyStartTlsSMTPServerTest {
  ...
  private ProtocolServer createServer(Protocol protocol, Encryption enc) {
    NettyServer server =
        new NettyServer.Factory(hashedWheelTimer)
            .protocol(protocol)
            .secure(enc)
            // .frameHandlerFactory(new AllButStartTlsLineChannelHandlerFactory("starttls",
            // AbstractChannelPipelineFactory.MAX_LINE_LENGTH))
            .frameHandlerFactory(
                new AllButStartTlsLineChannelHandlerFactory(
                    AbstractChannelPipelineFactory.MAX_LINE_LENGTH))
            .build();
  ...
  private static class StartTLSAssert implements AssertDelegateTarget {
    ...
    public boolean isStartTLSAnnounced() {
      return Arrays.stream(client.getReplyStrings())
          .anyMatch(reply -> reply.toUpperCase(Locale.US).endsWith("STARTTLS"));
    }
  }
  ...
  void startTlsShouldWorkWhenUsingJavamail() throws Exception {
    ...
    server =
        createServer(
            createProtocol(Optional.<ProtocolHandler>of(hook)),
            Encryption.createStartTls(BogusSslContextFactory.getServerContext()));
    server.bind();
